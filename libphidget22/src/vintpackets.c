...
recv1014_3_VINT(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_1014_DIGITALOUTPUT_800_VINT:
		...
		case VINT_PACKET_TYPE_DIGITALOUTPUT_FAILSAFE:
			return (bridgeSendToChannel(ch, BP_ERROREVENT, 2, "%d%s", EEPHIDGET_FAILSAFE, "Failsafe procedure initiated."));
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
recvVCP1002(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_VCP1002_VOLTAGEINPUT_110:
		...
		case VINT_PACKET_TYPE_VOLTAGEINPUT_SATURATION:
			return (bridgeSendToChannel(ch, BP_ERROREVENT, 2, "%d%s", EEPHIDGET_SATURATION, "Saturation Detected."));
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
recvTMP1101_1(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_TMP1101_VOLTAGEINPUT_200:
		...
		case VINT_PACKET_TYPE_VOLTAGEINPUT_BADCONNECTION:
			return (bridgeSendToChannel(ch, BP_ERROREVENT, 2, "%d%s", EEPHIDGET_BADCONNECTION, "Bad Connection"));
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
#if PHIDUID_STC1003_2_SUPPORTED
	case PHIDCHUID_STC1003_STEPPER_300:
#endif
		switch (bp->vpkt) {
		case BP_SETTARGETPOSITION:
			pack64(buf, (uint64_t)getBridgePacketInt64(bp, 0));
...
#if PHIDUID_STC1003_2_SUPPORTED
	case PHIDCHUID_STC1003_VOLTAGEINPUT_300:
#endif
		switch (bp->vpkt) {
		case BP_SETDATAINTERVAL:
			pack16(buf, HANDLE_DATAINTERVAL_PKT(bp, 1));
...
recvSTC1003_1(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_STC1003_STEPPER_200:
		...
		case VINT_PACKET_TYPE_STEPPER_POWER_ERROR:
			if(buf[0])
				return (bridgeSendToChannel(ch, BP_ERROREVENT, 2, "%d%s", EEPHIDGET_BADPOWER,	"Supply Voltage above expected range"));
			else
				return (bridgeSendToChannel(ch, BP_ERROREVENT, 2, "%d%s", EEPHIDGET_BADPOWER, "Supply Voltage below expected range. Possible overcurrent condition."));
		default:
			MOS_PANIC("Unexpected packet type");
		}
	case PHIDCHUID_STC1003_VOLTAGEINPUT_200:
...
#if PHIDUID_STC1003_2_SUPPORTED
static PhidgetReturnCode
sendSTC1003_2(PhidgetChannelHandle ch, BridgePacket *bp) {
	return sendSTC1003_1(ch, bp);
}

static PhidgetReturnCode
recvSTC1003_2(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	int64_t motorPosition;
	double velocity;
	double voltage;
	int stopped;
	...
	case PHIDCHUID_STC1003_STEPPER_300:
		...
		case VINT_PACKET_TYPE_STEPPER_FAULT_ERROR:
			return (bridgeSendToChannel(ch, BP_ERROREVENT, 2, "%d%s", EEPHIDGET_FAULT, "Fault condition detected. Motor is disengaged and position is lost. Re-engage and re-initialize your system."));
		case VINT_PACKET_TYPE_STEPPER_OVERTEMPERATURE:
			bridgeSendToChannel(ch, BP_ERROREVENT, 2, "%d%s", EEPHIDGET_FAULT, "Fault condition detected. Motor is disengaged and position is lost. Re-engage and re-initialize your system.");
			return (bridgeSendToChannel(ch, BP_ERROREVENT, 2, "%d%s", EEPHIDGET_OVERTEMP, "Overtemperature Condition Detected."));
		default:
			MOS_PANIC("Unexpected packet type");
		}
	case PHIDCHUID_STC1003_VOLTAGEINPUT_300:
...
recvSTC1001(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_STC1001_STEPPER_100:
		...
		case VINT_PACKET_TYPE_MOTORCONTROLLER_FAILSAFE:
			return (bridgeSendToChannel(ch, BP_ERROREVENT, 2, "%d%s", EEPHIDGET_FAILSAFE, "Failsafe procedure initiated."));
		case VINT_PACKET_TYPE_STEPPER_POWER_ERROR:
			return (bridgeSendToChannel(ch, BP_ERROREVENT, 2, "%d%s", EEPHIDGET_BADPOWER,
...
recvSAF1000(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_SAF1000_POWERGUARD_100:
		...
		case VINT_PACKET_TYPE_POWERGUARD_ENERGYDUMP:
			return (bridgeSendToChannel(ch, BP_ERROREVENT, 2, "%d%s", EEPHIDGET_ENERGYDUMP, "Energy Dump in Progress."));
			break;
		...
		case VINT_PACKET_TYPE_POWERGUARD_OVERVOLTAGE:
			if (buf[0])
				return (bridgeSendToChannel(ch, BP_ERROREVENT, 2, "%d%s", EEPHIDGET_OVERVOLTAGE, "Overvoltage Condition Detected."));
		case VINT_PACKET_TYPE_POWERGUARD_POWERON:
			return (bridgeSendToChannel(ch, BP_ERROREVENT, 2, "%d%s", EEPHIDGET_OK, "All error conditions have been resolved."));
			break;
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
recvMOT1101(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
		...
		case VINT_PACKET_TYPE_AXES_AXISVALUECHANGE:
			...
			if (fireSaturation == 0) {
				...
				if (fireSaturation & 0x01)
					ret = bridgeSendToChannel(ch, BP_ERROREVENT, 2, "%d%s", EEPHIDGET_SATURATION, "Accelerometer Saturation Detected.");
				if (fireSaturation & 0x02)
					ret = bridgeSendToChannel(ch, BP_ERROREVENT, 2, "%d%s", EEPHIDGET_SATURATION, "Gyroscope Saturation Detected.");
				if (fireSaturation & 0x04)
					ret = bridgeSendToChannel(ch, BP_ERROREVENT, 2, "%d%s", EEPHIDGET_SATURATION, "Magnetometer Saturation Detected.");
				else
	...
	case PHIDCHUID_MOT1101_ACCELEROMETER_100:
		...
		case VINT_PACKET_TYPE_AXES_AXISVALUECHANGE:
			...
			break;
		case VINT_PACKET_TYPE_AXES_SATURATION:
			return (bridgeSendToChannel(ch, BP_ERROREVENT, 2, "%d%s", EEPHIDGET_SATURATION, "Acceleration Saturation"));
			break;
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
recvMOT1102(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_MOT1102_SPATIAL_300:
		...
		case VINT_PACKET_TYPE_AXES_INVALID:
			return (bridgeSendToChannel(ch, BP_ERROREVENT, 2, "%d%s", EEPHIDGET_INVALIDSTATE, "The gyroscope has received invalid data and will recover shortly."));
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
recvMOT0110_VINT(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_MOT0110_SPATIAL_100_VINT:
		...
		case VINT_PACKET_TYPE_AXES_AXISVALUECHANGE:
			...
			if (len > 40) {
				if (buf[40] & 0x20)
					ret = bridgeSendToChannel(ch, BP_ERROREVENT, 2, "%d%s", EEPHIDGET_OUTOFRANGE, "One or more spatial readings is out or range");
			}
	...
	case PHIDCHUID_MOT0110_MAGNETOMETER_100_VINT:
		...
		case VINT_PACKET_TYPE_AXES_AXISVALUECHANGE:
			if (len > 16) {
				if (buf[16] & 0x04)
					ret = bridgeSendToChannel(ch, BP_ERROREVENT, 2, "%d%s", EEPHIDGET_OUTOFRANGE, "One or more magnetometer readings is out or range");
			}
	...
	case PHIDCHUID_MOT0110_GYROSCOPE_100_VINT:
		...
		case VINT_PACKET_TYPE_AXES_AXESUPDATE:
			if (len > 16) {
				if (buf[16] & 0x02)
					ret = bridgeSendToChannel(ch, BP_ERROREVENT, 2, "%d%s", EEPHIDGET_OUTOFRANGE, "One or more gyroscope readings is out or range");
			}
	...
	case PHIDCHUID_MOT0110_ACCELEROMETER_100_VINT:
		...
		case VINT_PACKET_TYPE_AXES_AXISVALUECHANGE:
			if (len > 16) {
				if (buf[16] & 0x01)
					ret = bridgeSendToChannel(ch, BP_ERROREVENT, 2, "%d%s", EEPHIDGET_OUTOFRANGE, "One or more accelerometer readings is out or range");
			}
	...
	case PHIDCHUID_MOT0110_TEMPERATURESENSOR_100_VINT:
		...
		case VINT_PACKET_TYPE_TEMPERATURESENSOR_TEMPERATURECHANGE:
			if (len > 4) {
				switch (buf[4] & 0x18) {
				case 0x08:
					ret = bridgeSendToChannel(ch, BP_ERROREVENT, 2, "%d%s", EEPHIDGET_OUTOFRANGELOW, "Temperature is too low to be accurately measured");
					break;
				case 0x10:
					ret = bridgeSendToChannel(ch, BP_ERROREVENT, 2, "%d%s", EEPHIDGET_OUTOFRANGEHIGH, "Temperature is too high to be accurately measured");
					break;
				case 0x18:
					ret = bridgeSendToChannel(ch, BP_ERROREVENT, 2, "%d%s", EEPHIDGET_OUTOFRANGE, "Temperature is unknown");
					break;
				}
			}
			temperature = unpackfloat(buf);
...
recvMOT0100_VINT(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_MOT0100_ACCELEROMETER_100_VINT:
		...
		case VINT_PACKET_TYPE_AXES_AXISVALUECHANGE:
			if (len > 16) {
				if (buf[16] & 0x01)
					bridgeSendToChannel(ch, BP_ERROREVENT, 2, "%d%s", EEPHIDGET_OUTOFRANGE, "One or more accelerometer readings is out or range");
			}
			accel[0] = unpackfloat(buf);
			accel[1] = unpackfloat(buf + 4);
			accel[2] = unpackfloat(buf + 8);
...
recvDST1000(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_DST1000_DISTANCESENSOR_100:
		...
		case VINT_PACKET_TYPE_DISTANCESENSOR_SATURATION:
			...
			return (bridgeSendToChannel(ch, BP_ERROREVENT, 2, "%d%s", EEPHIDGET_OUTOFRANGE, "Sensor value is ouside the valid range for this sensor."));
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
recvDCC1100(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_DCC1100_BLDCMOTOR_100:
		...
		case VINT_PACKET_TYPE_BLDCMOTOR_STALLDETECTED:
			return (bridgeSendToChannel(ch, BP_ERROREVENT, 2, "%d%s", EEPHIDGET_MOTORSTALL, "Dangerous motor stall detected. Velocity has been reduced in order to prevent damage."));
		default:
			MOS_PANIC("Unexpected packet type");
		}
	case PHIDCHUID_DCC1100_BLDCMOTOR_120:
	...
	case PHIDCHUID_DCC1100_MOTORPOSITIONCONTROLLER_100:
		...
		case VINT_PACKET_TYPE_MOTORPOSITIONCONTROLLER_MOTORSTATUSUPDATE:
			...
			return(bridgeSendToChannel(ch, BP_DUTYCYCLECHANGE, 1, "%g", dutyCycle));
		case VINT_PACKET_TYPE_BLDCMOTOR_STALLDETECTED:
			return (bridgeSendToChannel(ch, BP_ERROREVENT, 2, "%d%s", EEPHIDGET_MOTORSTALL, "Dangerous motor stall detected. Motor has been disengaged in order to prevent damage."));
		default:
			MOS_PANIC("Unexpected packet type");
		}
	case PHIDCHUID_DCC1100_MOTORPOSITIONCONTROLLER_120:
...
recvOUT1002(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_OUT1002_VOLTAGEOUTPUT_100:
		...
		case VINT_PACKET_TYPE_VOLTAGEOUTPUT_VOLTAGEERROR:
			return (bridgeSendToChannel(ch, BP_ERROREVENT, 2, "%d%s", EEPHIDGET_VOLTAGEERROR, "Voltage Error Detected"));
		default:
			MOS_PANIC("Unexpected packet type");
		}
	case PHIDCHUID_OUT1002_VOLTAGEOUTPUT_110:
...
recvCURLOOP(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_CURLOOP_CURRENTOUTPUT_100:
		...
		case VINT_PACKET_TYPE_CURRENTOUTPUT_CURRENTERROR:
			return (bridgeSendToChannel(ch, BP_ERROREVENT, 2, "%d%s", EEPHIDGET_BADPOWER, "Current Error Detected"));
		default:
			MOS_PANIC("Unexpected packet type");
		}
